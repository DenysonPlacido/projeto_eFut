...
--        ELSE
--        BEGIN
--            RAISERROR('N�o h� vagas dispon�veis para o tipo de jogador selecionado.', 16, 1);
--        END
--    END
--    ELSE  
--    BEGIN
...
BEGIN
    ...
    -- Verificar se o jogador j� est� na lista
    IF EXISTS (SELECT 1 FROM dbo.LISTA_JOGOS WHERE id_jogo = @idJogo AND id_jogador = @idJogador)
    BEGIN
        RAISERROR('O jogador j� est� na lista.', 16, 1);
        RETURN;
    END

    -- Obter as vagas dispon�veis no jogo
    SELECT @vagasGoleiros = vagas_gol, 
           @vagasLinhas = vagas_linhas, 
           @vagasSuplentes = vagas_suplentes
    FROM dbo.jogos
    ...
    -- Contar o n�mero de jogadores atuais
    SELECT @totalGoleiros = COUNT(*)
    FROM dbo.LISTA_JOGOS
    WHERE id_jogo = @idJogo AND TIPO_JOGADOR = 'Goleiro';
...
BEGIN
    ...
    -- Obter o tipo de jogador removido e a posi��o
...
BEGIN
	...
	SELECT 
		...
		LJ.DATA_HORA_REGISTRO AS 'Data e Hora da Endrada'
		--ISNULL(T.COR,'N�o Sorteado') AS 'Time'
	FROM dbo.LISTA_JOGOS LJ
		LEFT JOIN dbo.JOGADORES J
			ON LJ.ID_JOGADOR = J.ID_JOGADOR
	WHERE LJ.TIPO_JOGADOR = 'Linha'
...
BEGIN
	...
	SELECT 
		LJ.POSICAO_LISTA as 'Posi��o na Lista',
...
BEGIN
    ...
    -- Verifica��o de entrada
    IF NOT EXISTS (SELECT 1 FROM dbo.JOGOS WHERE ID_JOGO = @IDJOGO)
    BEGIN
        RAISERROR('Jogo n�o encontrado.', 16, 1);
        RETURN;
    END

    -- Inicializa��o e uso de vari�veis
    ...
    IF LEN(@CORES) = 0
    BEGIN
        RAISERROR('Lista de cores inv�lida.', 16, 1);
        RETURN;
    END

    -- Inser��o com controle de erros
    BEGIN TRY
    ...
    BEGIN CATCH
        ...
        -- Lan�amento do erro com a mensagem capturada
...
BEGIN
    ...
    -- Verifica se o jogador j� realizou o sorteio neste jogo
    IF EXISTS (
        SELECT 1
        FROM dbo.TAMPINHAS
        WHERE ID_JOGO = @IDJOGO AND ID_JOGADOR = @IDJOGADOR
    ...
    BEGIN
        -- O jogador j� realizou o sorteio; encerra a procedure
        PRINT 'O jogador j� realizou um sorteio para este jogo.';
        RETURN;
    END

    -- Declara a vari�vel para armazenar a tampinha sorteada
    DECLARE @TAMPINHAID INT;

    -- Seleciona uma tampinha dispon�vel aleatoriamente
    ...
    ELSE
    BEGIN
        PRINT 'N�o h� tampinhas dispon�veis para sorteio neste jogo.';
    END
END;
GO
...
BEGIN
    ...
    -- Executa a consulta din�mica com par�metros
...
BEGIN
	...
	('Ativo', null, @NOME, @APELIDO, @WHATS, @SENHA,@user_adm);
	...
	BEGIN CATCH
		...
		PRINT 'C�digo do erro: ' + CAST(ERROR_NUMBER() AS VARCHAR);
